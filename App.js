...
function AppShell() {
  ...
  const allTabsSeen = ['today', 'lists', 'done', 'settings'].every((k) => tabsSeen[k]);
  useEffect(() => {
  ...
  let screen;
  if (tab === 'today') {
    screen = <TodayScreen onOpenTask={openEdit} onOpenSearch={() => setSearchOpen(true)} />;
  } else if (tab === 'lists') {
    screen =
    ...
    screen = <DoneScreen onOpenTask={openEdit} />;
  } else {
    screen = (
      <SettingsScreen
        onToast={showToast}
